// R27 -- Latched death. The latch_field operator holds wall_state
// independent of memory: once set, only an explicit "heal" can
// release it. The spiral's structure outlives the spiral.
import init, { WasmCoupledR27 } from "../viewer/pkg/flow_wasm.js";
import {
    fitCanvas,
    drawField2D,
...
function buildSim() {
    try {
        if (sim) sim.free?.();
        sim = new WasmCoupledR27(
            W, H, DIFFUSION, A_VAL, B_VAL, BASE_EPS, DX, DT,
            killEpsVal, setThrVal, resetThrVal, leakVal,
        );
        sim.seed_spiral();
...
function frame() {
    if (sim) {
        ...
        drawField2D(wallCtx, f3.width, f3.height, sim.wall_field(), w, h, 1.0);
...
els.setThr.addEventListener("input", () => {
    ...
    sim?.set_set_threshold(setThrVal);
});
els.resetThr.addEventListener("input", () => {
    ...
    sim?.set_reset_threshold(resetThrVal);
});
