// R28 -- Communication. No new operator: latch + advect + latch
// composes into an information channel. wall_local latches as in
// R27. transmitted is wall_local advected rightward and
// re-latched (one-way) so every upstream commitment accumulates
// downstream. eps is driven by transmitted, not wall_local --
// each cell experiences walls that originated some time ago at
// position x - v*t. Walls travel without their builder.
import init, { WasmCoupledR28 } from "../viewer/pkg/flow_wasm.js";
import {
    fitCanvas,
    drawField2D,
...
function frame() {
    if (sim) {
        ...
        els.rWlR.textContent = (100 * sim.wall_local_right_fraction()).toFixed(1) + "%";
        els.rTrR.textContent = (100 * sim.transmitted_right_fraction()).toFixed(1) + "%";
...
els.sparkLeft.addEventListener("click",
    () => sim?.kick(Math.floor(W / 4),       H >> 1, 6, 1.0));
els.sparkRight.addEventListener("click",
    () => sim?.kick(Math.floor((3 * W) / 4), H >> 1, 6, 1.0));
